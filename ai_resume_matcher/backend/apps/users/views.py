...
class ResumeUploadView(APIView):
    ...
    def post(self, request):
        ...
        # After saving, compute skill gaps and suggest modules
        try:
            from training.views import SuggestedModulesView
            # Reuse logic by crafting a minimal request-like object is overkill; call matching directly
            from match.matching import extract_skills_from_text
            jd_skills = extract_skills_from_text((profile.summery or '').lower())
            prof_skills = [s.strip().lower() for s in re.split(r'[,•\n]+', profile.skills)] if profile.skills else []
            missing = [s for s in jd_skills if s not in prof_skills]
        except Exception:
            missing = []

        return Response({
            **Profileserilizer(profile).data,
            'training_missing_skills': missing,
